...
![version](https://img.shields.io/badge/version-v1.0-brown) ![python](https://img.shields.io/badge/python-3.8%2B-blue) ![license](https://img.shields.io/badge/license-MIT-green)
...
- **網頁版**（`app.py`）
  ...
  - 歷史紀錄表格，可依分店篩選
  - 前端每 15 秒自動刷新（免手動重整）
- **CLI 版**（`monitor.py`）：僅監控新竹店，純終端機輸出
...
├── monitor.py                # 終端機 CLI 版（新竹店）
├── index.html                # 前端頁面（Chart.js + vanilla JS）
├── all_branches_log.csv      # 全分店資料記錄
├── hsinchu_wait_log.csv      # 新竹店歷史（monitor.py 產生）
└── README.md
```
